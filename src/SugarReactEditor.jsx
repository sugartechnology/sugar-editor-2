...
function App({ viewDomElement }) {
  ...
  const [dockState, _] = useState(
    Dockable.useDockable((state) => {
  ...
  if (!isAuth) return <LoginScreen setIsAuth={setIsAuth} />;

  return (
    <div id="app">
      <Menu dockState={dockState} handleLogout={handleLogout} views={views} />
      <div
        style={{
          width: "100vw",
          height: "95vh",
    ...
    <div id="dockableMenu">
      ...
      >
        ...
        {views.map((view, index) => (
          <div
            key={index}
            className="dockableMenuOptions"
            onClick={() => {
              setIsOpen(false);
              Dockable.spawnFloating(dockState, view.component);
            }}
          >
            <GoPlus />
            {view.name}
        ...
        <div className="dockableMenuOptions" onClick={() => handleLogout()}>
          <CiLogout />
          Log out
        </div>
      </div>

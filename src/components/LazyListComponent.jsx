...
export default function LazyListComponent({
    ...
    const fetchItems = async (pageNumber, searchKey) => {
        setLoading(true);
        try {
            const data = searchKey ? await apiFunction(String(pageNumber), searchKey) : await apiFunction(String(pageNumber));
    ...
    return (
        <div id='productContainer' ref={inputref}>
            ...
            <div id="productList" style={{ maxHeight: maxHeight && maxHeight, height: height && height, position: "relative" }} className={viewType !== 0 ? (viewType === 1 ? "dual" : "trio") : ""}>
